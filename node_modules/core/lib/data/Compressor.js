...
Compressor.prototype.compress = function(data, callback, compressLevel){
    ...
    var that = this;
    zlib.gzip(data, {level: compressLevel}, function(err, compressedData) {
        callback(
            compressedData,
            that._getCompressionRate(data.length, compressedData.length)
        );
    });
};
...
Compressor.prototype.uncompress = function(data, callback){
    ...
    zlib.gunzip(data, function(err, decoded) {
        callback(
            decoded,
            that._getCompressionRate(data.length, decoded.length)
        );
    });
};
